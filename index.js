...
async function getTemplate(templateId) {
    ...
    let response = await fetch(`https://api.wolfeocrm.com/v1/setting/2/template/${templateId.formId}`, {
        ...
        headers: {
            ...
            'authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJMVENCV1M4RkRVTkdTMDNUIiwiaXNzIjoiaHR0cHM6Ly9hcGkuaWxvZ2dlcnouY29tL2FwaS92MS9sb2dpbiIsImlhdCI6MTY3Nzc2MTE5NSwiZXhwIjoxNjc3NzY0Nzk1LCJuYmYiOjE2Nzc3NjExOTUsImp0aSI6IkxUQ0JXUzhGRFVOR1MwM1QxNjc3NzYxMTk1In0.HzjpAEaKARVyoDj1bFiUmW4Rs1sFllUQco5QlCARTds'
        }
    }).then(async (res) => {
        data = await res.json();
    })
    ...
    for (var i = 0; i < allData.length; i++) {
        ...
        for (var j = 0; j < data.attributes.data[allData[i]].fields.length; j++) {
            ...
            if (data.attributes.data[allData[i]].fields[j] == data.attributes.data[allData[i]][data.attributes.data[allData[i]].fields[j]].fieldName) {
                ...
                paragraph.textContent = data.attributes.data[allData[i]][data.attributes.data[allData[i]].fields[j]].fieldName;
